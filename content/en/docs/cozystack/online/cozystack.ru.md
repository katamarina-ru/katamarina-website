---
title: "Документация Katamarina на cozystack.ru"
linkTitle: "Документация Katamarina на cozystack.ru"
description: "Документация Katamarina на cozystack.ru"
---

### Команда Katamarina обеспечивает работу сайта https://cozystack.ru и разработку русскоязычной версии документации

Разработка ведется в рамках репозиториев https://github.com/katamarina-ru/cozystack-website и https://sourcecraft.dev/katamarina/katamarina-website

## Для использования online-документации - перейдите по соответствующим ссылкам:


| Документ        | Ссылка                                            |
|-----------------|---------------------------------------------------|
| Введение        | https://cozystack.ru/docs/v1.6/introduction/      |
| Установка       | https://cozystack.ru/docs/v1.6/install/           |
| Эксплуатация    | https://cozystack.ru/docs/v1.6/operations/        |
| Разработка      | https://cozystack.ru/docs/v1.6/development/       |
| API             | https://cozystack.ru/docs/v1.6/cozystack-api/     |
