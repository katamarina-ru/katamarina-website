...
Если нужно больше двух реплик, например чтобы держать по одному webhook pod рядом с каждым apiserver на control plane из пяти узлов, переопределите значение `replicas` через Package `cozystack.cozystack-engine` так же, как для любого другого компонента (см. [Компоненты]({{% ref "/docs/v1.5/operations/configuration/components" %}})):
